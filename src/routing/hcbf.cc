...
hcbf::hcbf(PacketPool* PP, MAC* mc, PacketBuffer* Bf, int NID, Statistics* St, Settings *S, God *G) : Routing(PP, mc, Bf, NID, St, S, G)
{
	...
	ranking = new CentralityApproximation(NID, Set->getNN(),G);//getNN()): total nodes number
...
void hcbf::ContactRemoved(double CTime, int NID)
{
	...
	//bool* encComm=this->SimGod->getCommunityFromCertainNode(NID);//seems useless for louvain
...
void hcbf::ReceptionLCandFS(Header *hd,Packet *pkt,int PID,double CurrentTime)//
{
	...
	int *myLocalCommunity;
	int *encLocalCommunity;//enc:received package
	bool **encFamiliarSets;
	struct PktDest *mySummaryVector;
	Packet *responsePacket;
	Header *responseHeader;
	int myCommunityID = labeling->getMyCommunity();
...
// 	allPackets = Buf->getAllPackets();
	...
	*/
	...
	myLocalUniqueRank= ranking->getLocalRankLouvain(CurrentTime, myLocalCommunity,myCommunityID);//UI
	...
	responsePacket = new HCBFSummaryLouvain(CurrentTime, allPackets[0], mySummaryVector, myLocalCommunity, myLocalRank, myGlobalUniqueRank,myLocalUniqueRank, 0);
...
void hcbf::ReceptionBubbleSummary(Header *hd, Packet *pkt, int PID, double CurrentTime)// sixth seventh
{
	...
	int myCommunityID = labeling->getMyCommunity();
	int encNodesCommunityID=SimGod->getCommunityID(encNodeID);

	#ifdef HCBF_DEBUG
		printf("%f: Node %d received a packet containing bubble information and summary vector with ID %d from node %d\n", CurrentTime, this->NodeID, PID, hd->GetprevHop());
	#endif
	/* Get packet contents */
	encNumPkts = ((HCBFSummaryLouvain *) pkt)->getNumPackets();
	encSummaryVector = ((HCBFSummaryLouvain *) pkt)->getSummaryVector();//info of all packets in sender's buf
	encLocalCommunity = ((HCBFSummaryLouvain *) pkt)->getLocalCommunity();

	encLocalUniqueRank = ((HCBFSummaryLouvain *) pkt)->getLocalRank();
	encGlobalUniqueRank = ((HCBFSummaryLouvain *) pkt)->getGlobalRank();
	encLocalRank=((HCBFSummaryLouvain *) pkt)->getLocalMobilityRank();
	...
	//myLocalRank = ranking->getLocalRank(CurrentTime, myLocalCommunity);
	myLocalUniqueRank=ranking->getLocalRankLouvain(CurrentTime, myLocalCommunity,myCommunityID);//local UI
//if(myLocalRank>500)
//printf("%f\n",myLocalRank);
	...
	for(i = 0; i < encNumPkts; i++)// Key part 
	{
		...
		if(myLocalCommunity[encSummaryVector[i].Dest]==myCommunityID)
		{
		...
		else if(myLocalCommunity[encNodeID]!=myCommunityID)//I-node is not in enc-node's community nor destination's
		{
		...
		//bool* MaxUICommunity= this->SimGod->getCommunityFromCertainNode(pktinfo->UI_node);
			if(myLocalCommunity[encSummaryVector[i].Dest]==myCommunityID)// this node and the destination node are in the same community
			{
			#ifdef HCBF_DEBUG
		...
		//bool* MaxLPCommunity= this->SimGod->getCommunityFromCertainNode(pktinfo->LP_node);
			...
			if( (encLocalCommunity[encSummaryVector[i].Dest]!=encNodesCommunityID&&myLocalCommunity[encSummaryVector[i].Dest]==myCommunityID)
			  ||(encLocalCommunity[encSummaryVector[i].Dest]==encNodesCommunityID&&myLocalCommunity[encSummaryVector[i].Dest]==myCommunityID&&myLocalRank>encLocalRank) )
			{
		...
		if(encLocalCommunity[encSummaryVector[i].Dest]!=encNodesCommunityID)
		{
		...
		if(encBetterCarrier && myGlobalUniqueRank > encGlobalUniqueRank && pktinfo->UI_node==this->NodeID && encLocalCommunity[encSummaryVector[i].Dest]!=encNodesCommunityID&& myLocalCommunity[encSummaryVector[i].Dest]!=myCommunityID)
		{
		#ifdef HCBF_DEBUG
		...
		//if(encBetterCarrier && myLocalRank > encLocalRank &&pktinfo->LP_node==this->NodeID && !(encLocalCommunity[encSummaryVector[i].Dest]&& !myLocalCommunity[encSummaryVector[i].Dest]))
		if(encBetterCarrier && myLocalUniqueRank > encLocalUniqueRank &&pktinfo->LP_node==this->NodeID && myLocalCommunity[encNodeID]==myCommunityID)
		{
		#ifdef HCBF_DEBUG
...
int *hcbf::getCommunity(double CurrentTime)
{
	return labeling->cloneLocalCommunity(CurrentTime);
}

int hcbf::getNCFHelper(int *destCommunity)
{
	return ranking->getContactsBetweenNodeCommunities(destCommunity);

}

int hcbf::getContactsNum(int *encCommunity)
{
	return ranking->getContactsBetweenNodeCommunities(encCommunity);
...
int hcbf::getMyCommunityID(void)
{
	return labeling->getMyCommunity();
