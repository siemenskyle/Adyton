...
void BubbleRap::ReceptionData(Header* hd, Packet* pkt, int PID, double CurrentTime, int RealID)//PID: the old packet_ID, may differ from "RealID" if Dupulicated  others
{
	...
	if(hd->GetNextHop() != this->NodeID)//How could it happened? is it a kind of error?
	{
		/* Garbage collection */
		if(pkt->AccessPkt() == false)// where does AccessPkt get set
		{
...
void BubbleRap::ReceptionLCandFS(Header *hd,Packet *pkt,int PID,double CurrentTime)//
{
	...
	labeling->updateBubble(encLocalCommunity, encFamiliarSets, hd->GetprevHop(), CurrentTime);///FFF check for CommonMember
...
void BubbleRap::ReceptionBubbleSummary(Header *hd, Packet *pkt, int PID, double CurrentTime)// sixth seventh
{
	...
	Node* encNode=this->SimGod->getNode(encNodeID);
...
printf("the numpkts from encPKT is : %d \n",encNumPkts);
//printf("testing my new method output:%d at node: %d,\n",this->SimGod->getNeighborAmount(this->NodeID),this->NodeID);
	...
	for(i = 0; i < encNumPkts; i++)// Key part 
	{
		...
		Buf->setMaxCBC(this->SimGod->getCBC(destLocalCommunity,encLocalCommunity,CurrentTime),encNodeID,PktID);
		Buf->setMaxNCF(this->SimGod->getNCF(destLocalCommunity,encNodeID),encNodeID,PktID);
		PacketEntry* pktinfo=Buf->getPktInfo(PktID);
		...
		{
			if(pktinfo->UI!=0||myGlobalRank>pktinfo->UI)
		...
		{
			if(pktinfo->LP!=0 || myLocalRank>pktinfo->LP)//may need to check the previous maxLP-node is belong to this community too.
		...
		else//I-node and enc-node are in the same non-destination community. need to check why enc-node is not in the destination community
		{
			...
			else if(myLocalRank > this->SimGod->getMaxUI(encNodeID))
			{
		...
		bool* MaxUICommunity= this->SimGod->getCommunityFromCertainNode(this->SimGod->getMaxUI_node(encNodeID));
		...
		bool* MaxLPCommunity= this->SimGod->getCommunityFromCertainNode(this->SimGod->getMaxLP_node(encNodeID));
		if(this->SimGod->getMaxLP(encNodeID) !=0 && MaxLPCommunity[encSummaryVector[i].Dest])
		{
		encBetterCarrier=false;
		}

		//if we have a new maxCBC/NCF node
		if(pktinfo->CBC_node==this->NodeID)//if(pktinfo->CBC_node!=encNodeID)
		encBetterCarrier=false;
		if(pktinfo->NCF_node==this->NodeID)//if(pktinfo->NCF_node!=encNodeID)
		encBetterCarrier=false;

		if(pktinfo->UI>encGlobalRank)
		{
		encBetterCarrier=false;
		IamBetterGlobally=true;
		}
		if(pktinfo->LP>encLocalRank)
		encBetterCarrier=false;



		/* bubbleRap forwarding */
		...
		else
		{
			...
			else if(encGlobalRank < myGlobalRank)//since neither the EncounteredNode or INode have the Destination Node in communities, so just check UI of these two nodes
			{
				encBetterCarrier = false;
				IamBetterGlobally = true;
			}
		...
		{
			printf("I am the better Carrier, so the sender node should send the relative packets to me \n");
...
void BubbleRap::ReceptionRequestVector(Header *hd, Packet *pkt, int PID, double CurrentTime)
{
	...
	{
		printf("there are packets in the ReRevector \n");
	...
	if(outgoing)
	{
		...
		{
		printf("sending those packets from the ReRevector \n");
...
void BubbleRap::SendPacket(double STime, int pktID,int nHop,int RepValue)// what is RepValue. it always 1
{
	...
	newPkt->getHeader()->SetNextHop(nHop);//nhop the the id of the node that is going to received this packet. this variable is used for receiving
	...
	if(CurrentN > 0)
	{
		/* Set access attribute to safely delete the packet later *///we only should delete this packet after all the node received this packet
...
bool *BubbleRap::getCommunity(void)
{
	return labeling->cloneLocalCommunity(this->SimGod->getSimTime());
...
{
	return labeling->getContactsNumHelper(destCommunity);

}

int BubbleRap::getNCFHelper(bool *destCommunity)
{
	return ranking->getContactsBetweenNodeCommunity(destCommunity);
